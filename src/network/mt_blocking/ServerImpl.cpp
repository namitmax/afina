...
void ServerImpl::ExecutableFunction(const int client_socket) {
    ...
    {
        std::unique_lock<std::mutex> _lock_thread(_mutex);
        _sockets.erase(client_socket);
        close(client_socket);
        if (_sockets.empty() && !running) {
            _wait_stop.notify_all();
        }
    }
}
...
void ServerImpl::OnRun() {
    ...
    Afina::Concurrency::Executor executor("client", 10, 2, 1 ,10000000000);
    while (running.load()) {
        ...
        // TODO: Start new thread and process data from/to connection
        {
            ...
            if (running) {
                if (executor.Execute(&ServerImpl::ExecutableFunction, this, client_socket)) {
